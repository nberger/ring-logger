...
(defmethod request-details :default
  ...
  (let [default-keys [:character-encoding
                     :content-length
                     :content-type
                     :query-string
                     :remote-addr
  ...
  (debug logger (str "Request details: " (select-keys req (if log-query-string? 
                                                            default-keys
                                                            (remove #{:query-string} default-keys)))))))
...
(defmethod exception :default
  ...
  (error logger
         ...
         (str (ansi/style "Uncaught exception " :red)
              (ansi/style (or (.getMessage ^Throwable throwable) "<No Message>") :bright :red)
              " processing request:"
              " for " remote-addr
...
(defmethod exception :no-color
  ...
  (error logger
         ...
         (str "Uncaught exception "
              (or (.getMessage ^Throwable throwable) "<No Message>")
              " processing request:"
              " for " remote-addr
              (when timing (str " in (" (get-total-time request) " ms)"))
              " - request was: " (pr-str (redact-request request options)))))
